...
const Footer = params => {
  const auth = params.auth || new Auth();
    ...
    <footer className="lh-title mv5 pv5 pl4 pr3 ph5-ns bt b--black-10">
      ...
      <div>
        {!auth.isAuthenticated() && (
          <button className="" onClick={auth.login.bind(this)}>
            Log In
          </button>
        )}
        {auth.isAuthenticated() && (
          <button className="" onClick={auth.logout.bind(this)}>
            Log Out
          </button>
        )}
      </div>
...
Footer.getInitialProps = async function(ctx) {
  const { req, data } = ctx;
  return {
    data: data,
    auth: new Auth(ctx)
